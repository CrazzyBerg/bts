...
public:
    ...
    virtual unsigned int getTxTime(uint64_t& time) const
	{
        unsigned int status;
        uint64_t ts;
        status = bladerf_get_timestamp((struct bladerf *)lb_dev, BLADERF_TX, &ts);
        time = ts;
        return status;
    }
    virtual unsigned int getRxTime(uint64_t& time) const
	{
        unsigned int status;
        uint64_t ts;
        status = bladerf_get_timestamp((struct bladerf *)lb_dev, BLADERF_RX, &ts);
        time = ts;
        return status;
    }
	...
	{ return 0; /* m_dev ? m_dev->setLoopback(name) : NotInitialized; */ }
    virtual unsigned int calibrate()
	{ return 0; /* m_dev->calibrate(); */ }
...
unsigned int BrfLibUsbDevice::setFrequency(uint64_t hz, bool tx)
{
    ...
    ::printf("FREQUENY FREUCNYE!!!!!! RXG %lld\n", hz);
...
unsigned int BrfLibUsbDevice::updateFpga(const NamedList& params)
{
	...
	if (status)
	    break;
	String fName;
	if (val == YSTRING("115") || val == YSTRING("40") || val == YSTRING("A4"))
...
unsigned int BrfLibUsbDevice::openDevice(bool claim, String* error)
{
	...
	// Nuand               0x2cf0 Product=0x5246
        // Nuand 2.0 micro xA4 0x2cf0 Product=0x5250
	if (!((desc.idVendor == 0x1d50 && desc.idProduct == 0x6066) ||
	    (desc.idVendor == 0x2cf0 && (desc.idProduct == 0x5246 || desc.idProduct == 0x5250))))
	    continue;
	m_dev = m_list[i];
...
unsigned int BrfInterface::init(const NamedList& params, String& error)
{
    ...
    if (!s_usbContextInit) {
        Lock lck(__plugin);
	    status = bladerf_open((struct bladerf **)&lb_dev, NULL);
        //bladerf_log_set_verbosity( BLADERF_LOG_LEVEL_VERBOSE );

        if (status == 0) {
            char tmpserial[64];
            firstGoodTxDone = 0;
            Debug(&__plugin,DebugAll,"Initialized libbladerf context");
            s_usbContextInit = true;
            //lusbSetDebugLevel();
            Debug(this,DebugAll,"Created device (%p) [%p]",lb_dev,this);

            status = bladerf_get_serial((struct bladerf *)lb_dev, (char *)&tmpserial);
            lb_serial = tmpserial;
            if (!status) {
                Output("bladeRF detected, attached to serial=%s", tmpserial);
            }

	    } else {
            Debug(this,DebugNote,"Failed to create device: %d %s [%p]",
                    status, bladerf_strerror(status),this);
        }
...
Message* BrfInterface::buildNotify(const char* status)
{
    Message* m = new Message("module.update",0,true);
    m->addParam("module",__plugin.name());
    completeDevInfo(*m,true);
    m->addParam("status",status,false);
    return m;
}
unsigned int BrfInterface::initialize(const NamedList& params)
{
    ...
    if (status != 0) {
        Debug(this,DebugAll,"Could not create RX interface [%p]",this);
        return 1;
    }
     ...
     *
     * HOW TO TUNE:
     * Define #define BRF_DEBUG at the top of this file and recompile and
     * install yate.
     *
     *
    ...
    if (status != 0) {
        Debug(this,DebugAll,"Could not create TX interface [%p]",this);
        return 1;
    }

    const String& bw = params["filter"];
    if (bw) {
        unsigned int bwtmp = bw.toInteger(1,0,1);
        if (bwtmp < 250000)
            bwtmp = 250000;

        status = bladerf_set_bandwidth((struct bladerf*)lb_dev, BLADERF_CHANNEL_RX(0), bwtmp, NULL);
        if (status != 0) {
            Debug(this,DebugAll,"Could not set RX filter bandwidth [%p]",this);
            return 1;
        }
        status = bladerf_set_bandwidth((struct bladerf*)lb_dev, BLADERF_CHANNEL_TX(0), bwtmp, NULL);
        if (status != 0) {
            Debug(this,DebugAll,"Could not set TX filter bandwidth [%p]",this);
            return 1;
        }
    }
    ...
    const String& sr = params["samplerate"];
    if (sr) {
        unsigned int srtmp = sr.toInteger(1,0,1);
        unsigned int sractual;
        if (srtmp < 2500000)
            srtmp = 2500000;

        status = bladerf_set_sample_rate((struct bladerf*)lb_dev, BLADERF_CHANNEL_RX(0), srtmp, NULL);
        if (status != 0) {
            Debug(this,DebugAll,"Could not set RX samplerate[%p]",this);
            return 1;
        }

        status = bladerf_set_sample_rate((struct bladerf*)lb_dev, BLADERF_CHANNEL_TX(0), srtmp, NULL);
        if (status != 0) {
            Debug(this,DebugAll,"Could not set TX samplerate[%p]",this);
            return 1;
        }
    }

    const String& tfreq = params["txfrequency"];
    if (tfreq) {
        unsigned long long tfreqtmp = tfreq.toInteger(1,0,1);
        if (tfreqtmp < 1000000)
            tfreqtmp = 925000000;

        status = bladerf_set_frequency((struct bladerf*)lb_dev, BLADERF_CHANNEL_TX(0), tfreqtmp);
        if (status != 0) {
            Debug(this,DebugAll,"Could not set TX frequency[%p]",this);
            return 1;
        }
    }

    const String& rfreq = params["rxfrequency"];
    if (rfreq) {
        unsigned long long rfreqtmp = rfreq.toInteger(1,0,1);
        if (rfreqtmp < 1000000)
            rfreqtmp = 936000000;

        status = bladerf_set_frequency((struct bladerf*)lb_dev, BLADERF_CHANNEL_RX(0), rfreqtmp);
        if (status != 0) {
            Debug(this,DebugAll,"Could not set RX frequency[%p]",this);
            return 1;
        }
    }

    status = bladerf_enable_module((struct bladerf*)lb_dev, BLADERF_CHANNEL_TX(0), true);

    if (status != 0) {
        Debug(this,DebugAll,"Could not enable TX interface [%p]",this);
        return 1;
    }

    status = bladerf_enable_module((struct bladerf*)lb_dev, BLADERF_CHANNEL_RX(0), true);

    if (status != 0) {
        Debug(this,DebugAll,"Could not enable RX interface [%p]",this);
        return 1;
    }
    ...
    // might need to send this at some point
    //m->addParam("tx_frequency",String((int)tfreqtmp));
    //m->addParam("tx_samplerate",String(srtmp));
    //m->addParam("tx_filter",String(bwtmp));
    Engine::dispatch(*m);
    TelEngine::destruct(m);

    return 0;//m_dev->initialize(params);
}
    ...
    float* powerScale)
{
    ...
    getTxTime(nowtime);
#ifdef BRF_DEBUG_TX
    printf("TX: when=%lld (RFtime %lld) [timediff %d]\n", when, nowtime, when - nowtime);
#endif
    if (when - nowtime < 2000) {
        printf("skipped %d\n", when - nowtime);
        return 0;
    }

    struct bladerf_metadata mdata;
    memset(&mdata, 0, sizeof(mdata));
    mdata.flags = 0;

    if (firstGoodTxDone)
        mdata.flags = BLADERF_META_FLAG_TX_UPDATE_TIMESTAMP;
    else
        mdata.flags = BLADERF_META_FLAG_TX_BURST_START;

    mdata.timestamp = when ;
    //mdata.flags = BLADERF_META_FLAG_TX_NOW;

    short *o_samples;
    o_samples=(short *)malloc(2*sizeof(short)*size);
    int i;
    for (i = 0; i < 2 * size; i++) {
        o_samples[i] = (samples[i] * 1500);
    }
    status = bladerf_sync_tx((struct bladerf*)lb_dev, o_samples, size, &mdata, 30000);
#ifdef BRF_DEBUG_TX
    printf("TX: ret=%d\n", status);
#endif
    if (status == 0)
...
unsigned int BrfInterface::recv(uint64_t& when, float* samples, unsigned int& size)
{
    ...
    mdata.timestamp = when;
    //mdata.flags = BLADERF_META_FLAG_RX_NOW;
    short *i_samples;
    i_samples=(short *)malloc(2*sizeof(short)*size);

    uint64_t timenow;

#ifdef BRF_DEBUG_RX
    getRxTime(timenow);
    printf("RX: req=%lld size=%d\n", when, size);
#endif
    status = bladerf_sync_rx((struct bladerf*)lb_dev, i_samples, size, &mdata, 100000);
#ifdef BRF_DEBUG_RX
    printf("RX: ret=%d timestamp=%lu (RFtime:%lu) \n", status, mdata.timestamp, timenow);
#endif
    if (status == 0) {
        int i;
        for (i = 0; i < 2 * size; i++) {
            samples[i] = ((float)i_samples[i])/2048.0f;
        }
    } else {
        if (status == -14) {
            // don't force it, let it catch up by itself
            getRxTime(timenow);
            when = timenow;
            status = 0;
        }
...
unsigned int BrfInterface::setFrequency(uint64_t hz, bool tx)
{
	...
	hz,brfDir(tx),this);
    uint64_t actual;
    unsigned int status = bladerf_set_frequency((struct bladerf*)lb_dev, tx ? BLADERF_CHANNEL_TX(0) : BLADERF_CHANNEL_RX(0), hz);
    if (status)
	return status;
    status = bladerf_get_frequency((struct bladerf*)lb_dev, tx ? BLADERF_CHANNEL_TX(0) : BLADERF_CHANNEL_RX(0), &actual);
    uint64_t freq = hz;

    if (actual == freq)
	return 0;
    int delta = actual - freq;
    Debug(this,DebugNote,"Set %s frequency requested=%u read=%u delta=%d [%p]",
	brfDir(tx),freq,actual,delta,this);
    return NotExact;
}
...
{
    uint64_t freq = 0;
    unsigned int status = bladerf_get_frequency((struct bladerf *)lb_dev, tx ? BLADERF_CHANNEL_TX(0) : BLADERF_CHANNEL_RX(0), &freq);
    if (status == 0)
	hz = freq;
    return status;
}
...
unsigned int BrfInterface::setSampleRate(uint64_t hz)
{
    ...
    uint32_t sactual = 0;
    unsigned int status = bladerf_set_sample_rate((struct bladerf*)lb_dev, BLADERF_CHANNEL_TX(0), srate, &sactual);
    if (status)
	return status;
    if (sactual != srate) {
    ...
    status = bladerf_set_sample_rate((struct bladerf*)lb_dev, BLADERF_CHANNEL_RX(0), srate, &sactual);
    if (sactual != srate) {
...
{
    uint32_t srate = 0;
    unsigned int status = bladerf_get_sample_rate((struct bladerf*)lb_dev, BLADERF_CHANNEL_TX(0), &srate);
    if (status == 0)
	hz = srate;
    return status;
}
...
unsigned int BrfInterface::setFilter(uint64_t hz)
{
    ...
    uint32_t band = hz;
    uint32_t actual = 0;
    unsigned int status = bladerf_set_bandwidth((struct bladerf *)lb_dev, BLADERF_CHANNEL_TX(0), hz, &actual);
    if (status)
	return status;
    ...
    status = bladerf_set_bandwidth((struct bladerf *)lb_dev, BLADERF_CHANNEL_RX(0), hz, &actual);
    if (status)
	return status;
    if (actual != band){
...
{
    uint32_t band = 0;
    unsigned int status = bladerf_get_bandwidth((struct bladerf *)lb_dev, BLADERF_CHANNEL_TX(0), &band);
    if (status == 0)
	hz = band;
    return status;
}
...
unsigned int BrfInterface::setRxGain(int val, unsigned port, bool preMixer)
{
    ...
    unsigned int status = 0;
//    if (preMixer)
//        status = bladerf_set_gain((struct bladerf *)lb_dev, BLADERF_CHANNEL_RX(0), val);
//
    return status;
}
...
unsigned int BrfInterface::setTxGain(int val, unsigned port, bool preMixer)
{
    ...
    unsigned int status;
//    if (preMixer)
//        status = bladerf_set_gain((struct bladerf *)lb_dev, BLADERF_CHANNEL_TX(0), val);
//
    return status;
}
...
void BrfInterface::completeDevInfo(NamedList& p, bool full, bool retData)
{
    ...
    if (full && m_dev) { // this evals false
	p.addParam("address",address(),false);
	p.addParam("speed",String(speedStr()).toLower());
	p.addParam("serial",serial(),false);
    }
}
    ...
    int* newVal) const
{
    unsigned int status = 0;
    if (tx)
	    val += 35; //experimentally derived to match
    if (lb_dev) {
	    val *= 10;
	    val += 60;
        if (tx == false) {
	    val -= 15;
	    bladerf_set_gain_mode((struct bladerf *)lb_dev, BLADERF_CHANNEL_RX(0),  BLADERF_GAIN_MGC);
       }
        status = bladerf_set_gain((struct bladerf*)lb_dev,
			tx ? BLADERF_CHANNEL_TX(0) : BLADERF_CHANNEL_RX(0), val);
    }
    return status;
}
